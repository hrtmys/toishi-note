...
- **rails test / rails test:system** — the actual regression suite. System tests are the only thing in this project that catches real Turbo/Stimulus behavior (frame scoping, event listener cleanup, Turbo Drive's body-replacement semantics) — see `docs/engineering/dev-environment.md`. Pure functions live under `test/javascript/` instead (`yarn test:js` below). `yarn build` runs first because `bin/rails test:system` on its own skips jsbundling's `javascript:build` Rake hook — a stale `app/assets/builds/application.js` silently masks JS changes and looks exactly like a broken Stimulus controller.
- **yarn test:js** (`node --test test/javascript/`) — unit tests for pure JS (text transforms, list markers, table conversion, diff rendering, modal queue). No browser, ~2s. Also runs in the scan_js CI job, so coverage moved here from system tests stays enforced on GitHub.
- **rubocop** — house style (`docs/engineering/coding-style.md`).
- **commentlint** (`bin/commentlint`) — the comment convention (English, one line preferred, three consecutive lines max) on newly written comments only; pre-existing blocks are grandfathered. Diffed against the merge-base, so CI checks out with full history for this step.
